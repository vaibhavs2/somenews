...
export default function DomainChoice(props: Props): Node {
  const domainRefernce = useRef<{domains: Domain, timeOut: TimeoutID | null}>({
    domains: [props.domains[0].domain],
    timeOut: null,
  });
  const {getDomains, setDomains} = props;
  const activeChips = domain => !!getDomains.find(value => value == domain);
  ...
  const addDomains = (domain: string) => {
    let timeout: TimeoutID | null = domainRefernce.current.timeOut;
